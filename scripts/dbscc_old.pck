CREATE OR REPLACE PACKAGE dbscc IS
    ...
    c_version     CONSTANT VARCHAR2(240) := '1.2.1';
    ...
    FUNCTION get_object(p_type IN VARCHAR2,
    ...
    FUNCTION object_exist(p_objtype IN VARCHAR2,
                          p_objname IN VARCHAR2) RETURN NUMBER;
...
CREATE OR REPLACE PACKAGE BODY dbscc IS
    ...
    FUNCTION object_exist(p_objtype IN VARCHAR2,
                          p_objname IN VARCHAR2) RETURN NUMBER IS
        vres NUMBER;
    BEGIN
        SELECT COUNT(*)
          INTO vres
    ...
    FUNCTION get_object(p_type IN VARCHAR2,
                        p_name IN VARCHAR2) RETURN CLOB IS
        res CLOB;
    BEGIN
    ...
    BEGIN
        IF c_savehistory THEN
            ...
            IF (vdata IS NOT NULL) AND
               ((volddata IS NULL) OR
               (dbms_lob.compare(vdata, volddata) <> 0)) THEN
    ...
    BEGIN
        vopdate  := SYSDATE;
        vmachine := get_hostname;
        SELECT SUM(save2history(objtype,
                                objname,
                                vmachine,
                                lockinfo,
                                vopdate))
